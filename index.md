...
|---|---|:--|---|---|
| 3/30 12PM ET | Quantifying Uncertainty for Aggregated Classification Pipelines | NLP has helped massively scale-up previously small-scale content analyses. Many social scientists train NLP classifiers and then measure social constructs (e.g sentiment) for millions of unlabeled documents which are then used as variables in downstream causal analyses. However, many sources of uncertainty can arise in pipelines that use text classifiers: (a) uncertainty in the annotations that come from ambiguous text, (b) uncertainty in trained supervised classifiers, (c) uncertainty in aggregating individual-level classifications at inference time, and (d) uncertainty in the final causal estimates. In this tutorial, we will walk through data and Python code of a research pipeline that analyzes the effects of social feedback on “moral outrage” expressions on Twitter. Along the way, we will highlight these four sources of uncertainty and explore statistical and computational strategies to quantify these uncertainties.| [Katherine Keith](kakeith.github.io) | [Uncertainty over Uncertainty](https://aclanthology.org/2020.nlpcss-1.13.pdf); [Moral Outrage](https://www.science.org/doi/pdf/10.1126/sciadv.abe5641) |
